      ...
      async (err, result) => {
        if (result.length > 0) {
          await Message.updateOne(
            ...
            .then(() => res.status(HttpStatus.OK).json({ message: 'Message sent sucesfully' }))
            .catch(err =>
              res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: 'Error occured during sending message' })
            );
        } else {
